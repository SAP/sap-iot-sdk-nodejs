...
class ThingTypeService {
  ...
  static getThingType(thingTypeName, queryParameters = {}, {
    ...
    const path = `/ThingConfiguration/v1/ThingTypes('${thingTypeName}')${queryString ? `?${queryString}` : ''}`;
    return this.request({
  ...
  static getThingTypesByPackage(packageName, queryParameters = {}, {
    ...
    const queryString = querystring.stringify(queryParameters);
    const path = `/ThingConfiguration/v1/Packages('${packageName}')/ThingTypes${queryString ? `?${queryString}` : ''}`;
    return this.request({
      url: `${this.navigator.configThing()}${path}`,
      headers,
      resolveWithFullResponse,
