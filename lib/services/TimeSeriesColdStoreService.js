...
class TimeSeriesColdStoreService {
  ...
  static getColdStoreTimeSeriesData(thingId, thingTypeName, propertySetId, fromTime, toTime, queryParameters = {}, {
    ...
    const queryString = querystring.stringify(enhancedQueryParameters);
    const path = `/Things('${thingId}')/${thingTypeName}/${propertySetId}?${queryString}`;
    return this.request({
      url: `${this.navigator.appiotColdstore()}${path}`,
      headers,
      resolveWithFullResponse,
      jwt,
      scopes,
