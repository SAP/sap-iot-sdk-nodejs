...
class LeonardoIoT {
     ...
     * @property {String} [configuration.scope] - configuration of custom scope for token creation
     * @returns {LeonardoIoT}
     */
    constructor(configuration) {
        if (configuration && configuration.uaa && configuration.endpoints) {
            this.authenticator = new Authenticator(configuration.uaa, configuration.xsuaa, configuration.scope);
            this.navigator = new Navigator(configuration.endpoints);
        } else if (typeof configuration === 'object' && configuration.scope){
            this.authenticator = new Authenticator(ConfigurationProvider.getCredentials(), ConfigurationProvider.getXsuaaService(), configuration.scope);
            this.navigator = new Navigator(ConfigurationProvider.getDestinations());
        } else {
