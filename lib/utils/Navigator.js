...
class Navigator {
    ...
    constructor(destinations) {
        ...
        if (!destinations || Object.keys(destinations).length < 1) {
            throw Error('Incomplete navigator configuration. Ensure a Leonardo IoT service instance binding or configure authentication options including endpoints ' +
            'via default-env.json file as described in the readme section of used SAP Leonardo IoT SDK');
        }
    ...
    getDestination(serviceName) {
        ...
        if (!destination) {
            throw new Error(`Unknown destination for service name: ${serviceName}`);
        }
        return destination;
    }
}
