/**
 * This example shows how to read the latest measurements of a single thing
 * and also how to write them directly into the time series store without using the ingestion pipeline.
 *
 * TASKS BEFORE START:
...
function generateTimeSeriesDataPayload(propertySet) {
  ...
  propertySet.Properties.results.forEach((property) => {
    let value;
...
async function runTimeSeriesDataOperations() {
  ...
  const propertySet = thingType.d.PropertySets.results.filter((pSet) => pSet.DataCategory === 'TimeSeriesData')[0];
