...
Independent of your runtime environment you always have the option to handover all configurations manually within the Leonardo IoT client instantiation. Please be aware that a full configuration requires tenant credentials as well as API endpoint definitions. The following example demonstrates this pattern:
```js
const LeonardoIoT = require('sap-leonardo-iot-sdk');
const client = new LeonardoIoT({
    // Mandatory configuration of Leonardo IoT subaccount credentials
    uaa: {
        clientid: "myClientId",
